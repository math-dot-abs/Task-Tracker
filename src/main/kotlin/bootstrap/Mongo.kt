...
fun Application.configureMongo() {
    ...
    mongoDatabase!!.createCollection("Task")
    val collection = mongoDatabase!!.getCollection("Task")
    if (collection.countDocuments() == 0L) {
        collection.insertOne(
            Task(
                id = ObjectId().toHexString(),
                title = "Create initial Task",
                author = "Abassin Saleh",
                description = "Create Task, which is set initially if the collection doesn't contain any Tasks",
